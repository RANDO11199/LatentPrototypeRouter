...
class MixtralConfig_LPR(MixtralConfig):
    model_type = "mixtral_lpr"
    def __init__(
        self,
...
class MixtralBlockSparseTop2MLP(nn.Module):
    def __init__(self, config: MixtralConfig_LPR):
        super().__init__()
...
class MixtralSparseMoeBlock_LPR(MixtralSparseMoeBlock):
    """
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        routing_weights, kl, selected_experts, router_logits, z_decoed = self.gate(hidden_states)
        ...
        return final_hidden_states, router_logits, kl


class MixtralDecoderLayer_LPR(MixtralDecoderLayer):
    def __init__(self, config: MixtralConfig_LPR, layer_idx: int):
        super().__init__(config, layer_idx)

        self.block_sparse_moe = MixtralSparseMoeBlock_LPR(config,layer_idx)
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        hidden_states, router_logits, kl = self.block_sparse_moe(hidden_states)
        hidden_states = residual + hidden_states
...
class MixtralModel_LPR(MixtralModel):
    """
    ...
    def __init__(self, config: MixtralConfig_LPR):
        ...
        self.layers = nn.ModuleList(
            [MixtralDecoderLayer_LPR(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class MixtralForCausalLM_LPR(MixtralForCausalLM):
    def __init__(self, config):
        super().__init__(config)
        self.model = MixtralModel_LPR(config)
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        return MoeCausalLMOutputWithPast_KL(
            loss=loss,
            aux_loss=aux_loss,
            logits=logits,
            net_loss=net_loss,
