...
class Qwen3MoeSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        #     np.save(f"{self.model_type}_{self.layer_idx}_expertkeys.npy", self.gate.weight.cpu().numpy()) 
        self._forward_count+=1
        batch_size, sequence_length, hidden_dim = hidden_states.shape
        hidden_states = hidden_states.view(-1, hidden_dim)
        # router_logits: (batch * sequence_length, n_experts)
        # np.save(f"{self.model_type}_encoded{self.layer_idx}_num_{self._forward_count}.npy", hidden_states.cpu().numpy())  
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        if output_router_logits:
            ...
            aux_loss = self.router_aux_loss_coef * aux_loss.to(loss.device)

        if self.training:
            if labels is not None:
                loss +=  aux_loss.to(loss.device) 
        else:
            print(f"Task Loss: {loss:.2f}, Aux Loss: {aux_loss:.2f}")
